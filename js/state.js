...
setNotifySignals(enabled) {
    ...
    clearSignals() {
        ...
        // Волны Эллиотта
    setElliottWaves(symbol, waves) {
        const key = `elliottWaves_${symbol}`;
        this.saveToStorage(key, waves);
    }

    getElliottWaves(symbol) {
        return this.loadFromStorage(`elliottWaves_${symbol}`, []);
    }

    clearElliottWaves(symbol) {
        const key = `elliottWaves_${symbol}`;
